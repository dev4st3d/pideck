...
pub(in crate::views) fn summary_card(
    ...
    div().w_full().border_t_1().border_color(theme::edge()).flex().flex_col()
        .child(div().w_full().h(px(47.0)).flex().items_center().justify_between().gap(px(12.0))
            .child(div().id("workspace-diff-files-toggle").tab_index(0).cursor_pointer()
                .h(px(30.0)).min_w_0().flex().items_center().gap(px(28.0))
                .text_color(theme::bone()).hover(|row| row.text_color(theme::focus()))
                .focus(|row| row.text_color(theme::focus()))
                .on_click(move |_, _, cx| toggle_root.update(cx, |view, cx| view.toggle_workspace_diff_files(cx)))
                .child(div().font_family(theme::sans()).text_size(theme::text_size(12.0))
                    .child(format!("{file_count} file{} changed", if file_count == 1 { "" } else { "s" })))
                .child(div().flex().items_center().gap(px(8.0))
                    .child(stat_text(format!("+{additions}"), theme::live()))
                    .child(stat_text(format!("−{deletions}"), theme::live())))
                .when(snapshot.counts_partial, |row| row.child(div()
                    .font_family(theme::sans()).text_size(theme::text_size(11.0))
                    .text_color(theme::ash()).child("partial counts"))))
            .child(div().id("workspace-diff-open").tab_index(0).cursor_pointer()
                .h(px(30.0)).pr(px(12.0)).flex().items_center().gap(px(26.0))
                .font_family(theme::sans()).text_size(theme::text_size(12.0)).text_color(theme::signal())
                .hover(|button| button.text_color(theme::signal_hot()))
                .focus(|button| button.bg(theme::selection()))
                .on_click(move |_, window, cx| open_root.update(cx, |view, cx| view.open_workspace_diff(window, cx)))
                .child("Review changes").child(svg().path("icons/external.svg").size(px(16.0)).text_color(theme::signal()))))
        .when(expanded, |card| card.child(div().w_full().pb(px(12.0)).flex().flex_col()
            .children(snapshot.files.iter().take(MAX_EXPANDED_FILES).enumerate()
                .map(|(index, file)| file_row(index, file, root.clone())))
            .when(snapshot.files.len() > MAX_EXPANDED_FILES, |list| list.child(div()
                .py(px(6.0)).font_family(theme::sans()).text_size(theme::text_size(11.0))
                .text_color(theme::ash()).child(format!("{} more files in Review changes", snapshot.files.len() - MAX_EXPANDED_FILES))))))
}
...
fn stat_text(text: String, color: gpui::Rgba) -> impl IntoElement {
    div()
        ...
        .text_size(theme::text_size(11.0))
